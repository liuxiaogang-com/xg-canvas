...
#   register → login → list/create project → quick-gen 4 task types →
#   confirm tasks reach succeeded → assets visible globally and in project.
#
# Prerequisites:
#   - docker compose up -d
#   - account-api seeded with at least one openai-compat credential
...
text_task=$(post POST "/tasks" "$(jq -n --argjson p "{}" '{task_type:"gen.text",model_id:"openai:gpt-4o-mini",params:{max_tokens:20},inputs:{prompt:"reply OK"}}')")
...
for i in $(seq 1 30); do
  ...
  echo "  status=$s"
  case "$s" in
    succeeded) pass "text task succeeded"; break ;;
