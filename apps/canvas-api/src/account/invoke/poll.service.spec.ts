...
describe('PollService historical routing and request logging', () => {
  it('logs before polling the exact historical route and keeps the poll row non-billable', async () => {
    ...
    harness.poll.mockResolvedValue(result);

    await expect(harness.service.poll(request())).resolves.toBe(result);
    ...
    expect(harness.poll).toHaveBeenCalledWith(
      EXTERNAL_TASK_ID,
    ...
    expect(harness.requestLogs.record.mock.invocationCallOrder[0]).toBeLessThan(
      harness.poll.mock.invocationCallOrder[0],
    );
    expect(harness.poll.mock.invocationCallOrder[0]).toBeLessThan(
  ...
  it('finalizes an adapter exception against the pending poll row before rethrowing', async () => {
    const harness = makeHarness();
    const error = new AdapterError({
      code: ERROR_CODES.VENDOR_UNAVAILABLE,
      message: 'vendor poll unavailable',
      retryable: true,
      httpStatus: 503,
      vendor: { request: 'vendor-request-1' },
    });
    harness.poll.mockRejectedValue(error);

    await expect(harness.service.poll(request())).rejects.toBe(error);
    ...
    expect(harness.requestLogs.finalizePending).toHaveBeenCalledWith(log.id, {
      ...
      error_message: 'vendor poll unavailable',
...
function request(): PollRequestDto {
  return {
    task_id: TASK_ID,
