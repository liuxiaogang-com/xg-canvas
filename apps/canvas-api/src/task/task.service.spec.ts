...
describe('TaskService.create source-node binding', () => {
  ...
  beforeEach(() => {
    ...
    service = new TaskService(repo, projects, workspaces, authz, ds, terminal, execution);
    (projects.getOrThrow as jest.Mock).mockResolvedValue({ id: PROJECT_ID, workspace_id: WORKSPACE_ID });
  ...
  it('rejects source_node_id without project_id', async () => {
    await expect(service.create(USER_ID, WORKSPACE_ID, dto({ source_node_id: NODE_ID }))).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it('rejects a project outside the active workspace', async () => {
    ...
    await expect(
      service.create(USER_ID, WORKSPACE_ID, dto({ project_id: PROJECT_ID, source_node_id: NODE_ID })),
  ...
  it('persists a task only after the project and node checks pass', async () => {
    ...
    expect(authz.can).toHaveBeenCalledWith(USER_ID, 'project.canvas.node.edit', 'project', PROJECT_ID);
    expect(manager.query).toHaveBeenCalledWith(expect.stringContaining('JOIN canvas.canvases'), [
      NODE_ID,
      PROJECT_ID,
      WORKSPACE_ID,
    ]);
    expect(task).toMatchObject({ project_id: PROJECT_ID, source_node_id: NODE_ID, workspace_id: WORKSPACE_ID });
  });

  it('claims work with a durable lease and compare-and-set token', async () => {
    (ds.query as jest.Mock).mockResolvedValueOnce([
      { id: 'task-1', status: 'queued', lease_token: 'lease-1', lease_expires_at: new Date(), attempt_no: 1 },
    ]);
  ...
  it('clears vendor routing and lease state before an automatic retry', async () => {
    ...
    expect(ds.query).toHaveBeenCalledWith(
      expect.stringMatching(/external_task_id = NULL[\s\S]+channel_id = NULL[\s\S]+lease_token = NULL/),
  ...
  it('retries result ingestion without clearing the paid vendor task id', async () => {
    ...
    await expect(
      service.schedulePollRetry('task-1', 'lease-1', 'ASSET_DOWNLOAD_FAILED', 'storage down', new Date()),
    ).resolves.toBe(true);
    ...
    expect(sql).not.toContain('channel_id = NULL');
  });
  ...
  it('re-authorizes project permissions before an explicit retry', async () => {
    ...
    await expect(service.retry(USER_ID, WORKSPACE_ID, 'task-1')).rejects.toBeInstanceOf(ForbiddenException);
    expect(ds.query).not.toHaveBeenCalledWith(expect.stringContaining('SET status'), expect.anything());
  });
  ...
  it('keeps task detail isolated to the active workspace', async () => {
    ...
    await expect(service.getOrThrow(USER_ID, 'task-1', WORKSPACE_ID)).rejects.toMatchObject({ status: 404 });
  });

  it('lists only active tasks that have not been superseded on the same node', async () => {
    await service.list(USER_ID, { workspace_id: WORKSPACE_ID, project_id: PROJECT_ID, active: true });
...
function dto(extra: { project_id?: string; source_node_id?: string }) {
  return {
    task_type: 'gen.image',
    model_id: 'test:model',
    params: {},
